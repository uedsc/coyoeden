    ...
    public class StoryRepositoryFixture : BaseIntegrationFixture
    {
        ...
        public void FindPromotedByUser_Should_Return_Correct_Result()
        {
            using (BeginTransaction())
            {
                ...
                stories.ForEach(s => s.StoryVote.Add(new StoryVote { User = newUser, Story = s, IPAddress = "192.168.0.5", Timestamp = SystemTime.Now() }));
                _database.SubmitChanges();
        ...
        public void FindCommentedByUser_Should_Return_Correct_Result()
        {
            using (BeginTransaction())
            {
                ...
                var rnd = new Random();
                stories.ForEach(s => s.StoryComment.Add((StoryComment)_domainFactory.CreateComment(s, "content", SystemTime.Now().AddDays(rnd.Next(-10, -1)), newUser, "192.168.0.1")));
                stories.ForEach(s => s.StoryComment.Add((StoryComment)_domainFactory.CreateComment(s, "content2", SystemTime.Now().AddDays(rnd.Next(-10, -1)), newUser, "192.168.0.4")));
                stories.ForEach(s => s.StoryComment.Add((StoryComment)_domainFactory.CreateComment(s, "content2", SystemTime.Now().AddDays(rnd.Next(-10, -1)), anotherUser, "192.168.0.4")));
                _database.SubmitChanges();
