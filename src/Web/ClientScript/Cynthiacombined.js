...
function Get_Cookie( check_name ) {
	...
	for ( i = 0; i < a_all_cookies.length; i++ )
	{
		...
		a_tecy_cookie = a_all_cookies[i].split( '=' );
		
		
		// and trim left/right whitespace while we're at it
		cookie_name = a_tecy_cookie[0].replace(/^\s+|\s+$/g, '');
		...
		if ( cookie_name == check_name )
		{
			...
			if ( a_tecy_cookie.length > 1 )
			{
				cookie_value = unescape( a_tecy_cookie[1].replace(/^\s+|\s+$/g, '') );
			}
