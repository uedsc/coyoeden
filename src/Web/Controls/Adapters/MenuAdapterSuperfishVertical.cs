...
/// addition of sf-vertical class on the ul is the only difference compared to MenuAdapterSuperfish
...
namespace Cynthia.Web
{
    ...
    public class MenuAdapterSuperfishVertical : System.Web.UI.WebControls.Adapters.MenuAdapter
    {
        ...
        private void BuildItems(MenuItemCollection items, bool isRoot, HtmlTextWriter writer)
        {
            if (items.Count > 0)
            {
                ...
                if (isRoot)
                {
                    // addition of sf-vertical class is the only difference compared to MenuAdapterSuperfish
                    writer.WriteAttribute("class", "sf-menu sf-vertical");
                }
