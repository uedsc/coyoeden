...
namespace Cynthia.Data
{
    public static class DBFriendlyUrl
    {
        ...
        public static bool DeleteByPageId(int pageId)
        {
            ...
            int rowsAffected = MySqlHelper.ExecuteNonQuery(
                GetWriteConnectionString(), sqlCommand.ToString(), null);
        ...
        public static IDataReader GetByUrl(string hostName, string friendlyUrl)
        {
            ...
            sqlCommand.Append("(SELECT SiteID From cy_SiteHosts WHERE HostName = ?HostName LIMIT 1),");
            sqlCommand.Append("(SELECT SiteID From cy_Sites ORDER BY SiteID LIMIT 1)");
            sqlCommand.Append(") ");
