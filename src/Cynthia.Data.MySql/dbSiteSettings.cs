...
namespace Cynthia.Data
{
    public static class DBSiteSettings
    {
            ...
            bool disableDbAuth)
        {
            ...
            sqlCommand.Append("INSERT INTO cy_Sites ( ");
            ...
            bool disableDbAuth)
        {
            ...
            StringBuilder sqlCommand = new StringBuilder();
            sqlCommand.Append("UPDATE cy_Sites ");
        ...
        public static void Delete(int siteId)
        {
            ...
            sqlCommand.Append("DELETE FROM cy_WebParts WHERE SiteID = ?SiteID; ");

            sqlCommand.Append("DELETE FROM cy_PageModules ");
            sqlCommand.Append("WHERE PageID IN (SELECT PageID FROM cy_Pages WHERE SiteID = ?SiteID); ");

            sqlCommand.Append("DELETE FROM cy_ModuleSettings WHERE ModuleID IN (SELECT ModuleID FROM cy_Modules WHERE SiteID = ?SiteID);");
       
            sqlCommand.Append("DELETE FROM cy_HtmlContent WHERE ModuleID IN (SELECT ModuleID FROM cy_Modules WHERE SiteID = ?SiteID);");

            sqlCommand.Append("DELETE FROM cy_Modules WHERE SiteID = ?SiteID; ");
            sqlCommand.Append("DELETE FROM cy_SiteModuleDefinitions WHERE SiteID = ?SiteID; ");
            sqlCommand.Append("DELETE FROM cy_UserProperties WHERE UserGuid IN (SELECT UserGuid FROM cy_Users WHERE SiteID = ?SiteID);");
            sqlCommand.Append("DELETE FROM cy_UserRoles WHERE UserID IN (SELECT UserID FROM cy_Users WHERE SiteID = ?SiteID);");
            sqlCommand.Append("DELETE FROM cy_UserLocation WHERE UserGuid IN (SELECT UserGuid FROM cy_Users WHERE SiteID = ?SiteID);");
            sqlCommand.Append("DELETE FROM cy_FriendlyUrls WHERE SiteID = ?SiteID; ");
            sqlCommand.Append("DELETE FROM cy_UserPages WHERE SiteID = ?SiteID; ");
            sqlCommand.Append("DELETE FROM cy_Users WHERE SiteID = ?SiteID; ");
            sqlCommand.Append("DELETE FROM cy_Pages WHERE SiteID = ?SiteID; ");
            sqlCommand.Append("DELETE FROM cy_Roles WHERE SiteID = ?SiteID; ");
            sqlCommand.Append("DELETE FROM cy_SiteHosts WHERE SiteID = ?SiteID; ");
            sqlCommand.Append("DELETE FROM cy_SitePersonalizationAllUsers WHERE PathID IN (SELECT PathID FROM cy_SitePaths WHERE SiteID = ?SiteID);");
            sqlCommand.Append("DELETE FROM cy_SitePersonalizationPerUser WHERE PathID IN (SELECT PathID FROM cy_SitePaths WHERE SiteID = ?SiteID);");
            sqlCommand.Append("DELETE FROM cy_SitePaths WHERE SiteID = ?SiteID; ");
            sqlCommand.Append("DELETE FROM cy_SiteFolders WHERE SiteGuid IN (SELECT SiteGuid FROM cy_Sites WHERE SiteID = ?SiteID);");
            sqlCommand.Append("DELETE FROM cy_SiteSettingsEx WHERE SiteID = ?SiteID; ");

            sqlCommand.Append("DELETE FROM cy_LetterSendLog   ");
            sqlCommand.Append("WHERE LetterGuid IN (SELECT LetterGuid FROM cy_Letter   ");
            sqlCommand.Append("WHERE LetterInfoGuid IN (SELECT LetterInfoGuid   ");
            sqlCommand.Append("FROM cy_LetterInfo   ");
            sqlCommand.Append("WHERE SiteGuid IN (SELECT SiteGuid FROM cy_Sites WHERE SiteID = ?SiteID) ");
            sqlCommand.Append("));");

            sqlCommand.Append("DELETE FROM cy_LetterSubscribeHx   ");
            sqlCommand.Append("WHERE LetterInfoGuid IN (SELECT LetterInfoGuid   ");
            sqlCommand.Append("FROM cy_LetterInfo   ");
            sqlCommand.Append("WHERE SiteGuid IN (SELECT SiteGuid FROM cy_Sites WHERE SiteID = ?SiteID)   ");
            sqlCommand.Append(");");

            sqlCommand.Append("DELETE FROM cy_LetterSubscribe  ");
            sqlCommand.Append("WHERE LetterInfoGuid IN (SELECT LetterInfoGuid   ");
            sqlCommand.Append("FROM cy_LetterInfo   ");
            sqlCommand.Append("WHERE SiteGuid IN (SELECT SiteGuid FROM cy_Sites WHERE SiteID = ?SiteID)   ");
            sqlCommand.Append(");");
            ...
            sqlCommand.Append("DELETE FROM cy_LetterHtmlTemplate WHERE SiteGuid IN (SELECT SiteGuid FROM cy_Sites WHERE SiteID = ?SiteID);");
            sqlCommand.Append("DELETE FROM cy_LetterInfo WHERE SiteGuid IN (SELECT SiteGuid FROM cy_Sites WHERE SiteID = ?SiteID);");

            sqlCommand.Append("DELETE FROM cy_AuthorizeNetLog WHERE SiteGuid IN (SELECT SiteGuid FROM cy_Sites WHERE SiteID = ?SiteID);");
            sqlCommand.Append("DELETE FROM cy_PlugNPayLog WHERE SiteGuid IN (SELECT SiteGuid FROM cy_Sites WHERE SiteID = ?SiteID);");
            sqlCommand.Append("DELETE FROM cy_GoogleCheckoutLog WHERE SiteGuid IN (SELECT SiteGuid FROM cy_Sites WHERE SiteID = ?SiteID);");
            sqlCommand.Append("DELETE FROM cy_PayPalLog WHERE SiteGuid IN (SELECT SiteGuid FROM cy_Sites WHERE SiteID = ?SiteID);");

            sqlCommand.Append("DELETE FROM cy_RedirectList WHERE SiteGuid IN (SELECT SiteGuid FROM cy_Sites WHERE SiteID = ?SiteID);");
            sqlCommand.Append("DELETE FROM cy_TaskQueue WHERE SiteGuid IN (SELECT SiteGuid FROM cy_Sites WHERE SiteID = ?SiteID);");
            sqlCommand.Append("DELETE FROM cy_TaxClass WHERE SiteGuid IN (SELECT SiteGuid FROM cy_Sites WHERE SiteID = ?SiteID);");
            sqlCommand.Append("DELETE FROM cy_TaxRateHistory WHERE SiteGuid IN (SELECT SiteGuid FROM cy_Sites WHERE SiteID = ?SiteID);");
            sqlCommand.Append("DELETE FROM cy_TaxRate WHERE SiteGuid IN (SELECT SiteGuid FROM cy_Sites WHERE SiteID = ?SiteID);");


            sqlCommand.Append("DELETE FROM cy_Sites ");
            sqlCommand.Append("WHERE SiteID = ?SiteID  ; ");
        ...
        {
            StringBuilder sqlCommand = new StringBuilder();
            sqlCommand.Append("SELECT Count(*) FROM cy_SiteModuleDefinitions ");
        ...
        public static void AddHost(Guid siteGuid, int siteId, string hostName)
        {
            ...
            sqlCommand.Append("INSERT INTO cy_SiteHosts ");
            sqlCommand.Append("( ");
            sqlCommand.Append("SiteID, ");
            sqlCommand.Append("SiteGuid, ");
        ...
        {
            StringBuilder sqlCommand = new StringBuilder();
            sqlCommand.Append("DELETE FROM cy_SiteHosts ");
        ...
        public static IDataReader GetSite(string hostName)
        {
            ...
            sqlCommand.Append("SELECT cy_SiteHosts.SiteID ");
            sqlCommand.Append("FROM cy_SiteHosts ");
