...
public partial class _default : SiteBasePage
{
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		else if (Request.QueryString.Count == 0 || !string.IsNullOrEmpty(Request.QueryString["page"]) || !string.IsNullOrEmpty(Request.QueryString["theme"]) || !string.IsNullOrEmpty(Request.QueryString["blog"]))
		{
			if (!BlogSettings.Instance.UseBlogNameInPageTitles)
				Page.Title = String.Format("{0} | ", BlogSettings.Instance.Name);
			...
			try
			{
				...
				if (docs.Count > 0)
				{
					foreach (Uri key in docs.Keys)
					{
						...
						PostList1.Posts.Sort(delegate(IPublishable ip1, IPublishable ip2) { return ip2.DateCreated.Value.CompareTo(ip1.DateCreated.Value); });
						Page.Title += String.Format(" for {0}", Server.HtmlEncode(key.Host));
						break;
					}
				}
				else
	...
	private void Redirect()
	{
		...
		{
			rewrite = String.Format("{0}{1}/{2}/default.aspx", Utils.RelativeWebRoot, year, month); 
		}
		else if (!string.IsNullOrEmpty(year))
		{
			rewrite = String.Format("{0}{1}/default.aspx", Utils.RelativeWebRoot, year);
		}
	...
	private void DisplayCategories()
	{
		if (!String.IsNullOrEmpty(Request.QueryString["id"]))
		{
			...
			Page.Title = Category.GetCategory(categoryId).Name;
		}
	}
