...
/// </summary>
[Extension("Converts BBCode to XHTML in the comments", "1.0", "<a href=\"http://vivasky.com\">CoyoEden.NET</a>")]
public class BBCode
{
    ...
    static BBCode()
    {
        PostComment.Serving += new EventHandler<ServingEventArgs>(Post_CommentServing);
    ...
    {
		int start = body.IndexOf(String.Format("[{0}]", code));
        if (start > -1)
        {
			if (body.IndexOf(String.Format("[/{0}]", code), start) > -1)
            {
				...
				int end = body.IndexOf(String.Format("[/{0}]", code), start);
