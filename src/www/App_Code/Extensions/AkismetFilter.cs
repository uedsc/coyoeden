...
/// </summary>
[Extension("Aismet anti-spam comment filter", "1.0", "<a href=\"http://vivasky.com\">CoyoEden.NET</a>")]
public class AkismetFilter : ICustomFilter
{
    ...
    public bool Initialize()
    {
        ...
        _api = new Akismet(_key, _site, "CoyoEden.net 1.5");

        return _api.VerifyKey();
    }

    public bool Check(IComment comment)
    {
        AkismetComment akismetComment = GetAkismetComment(comment);
        return _api.CommentCheck(akismetComment);
    }

    public void Report(IComment comment, bool isSpam)
    {
        AkismetComment akismetComment = GetAkismetComment(comment);
    ...
    private AkismetComment GetAkismetComment(IComment comment)
    {
