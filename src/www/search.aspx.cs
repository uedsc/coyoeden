...
public partial class search : CoyoEden.UI.SiteBasePage
{
	...
	void rep_ItemDataBound(object sender, RepeaterItemEventArgs e)
	{
		...
		if (e.Item.DataItem is IComment)
		{
		...
		else if (e.Item.DataItem is Post)
		{
			...
			if (post.Categories.Count > 0)
			{
				...
				{
					cat += String.Format("{0}, ", category.Name);
				}

				text += String.Format("<br />{0}", string.Format(categories, cat.Substring(0, cat.Length - 2)));
			}
			...
			if (post.Tags.Count > 0)
			{
				...
				{
					t += String.Format("{0}, ", tag);
				}

				text += String.Format("<br />{0}", string.Format(tags, t.Substring(0, t.Length - 2)));
			}

			control.InnerHtml = text;
		}
		else if (e.Item.DataItem is CoyoEden.Core.Page)
		{
			CoyoEden.Core.Page page = (CoyoEden.Core.Page)e.Item.DataItem;
			string text = string.Format(type, Resources.labels.page);

			if (!string.IsNullOrEmpty(page.Keywords))
			{
				text += String.Format("<br />{0}", string.Format(keywords, page.Keywords));
			}
	...
	private void BindPaging(int results, int page)
	{
		...
		for (int i = 0; i < pages; i++)
		{
			...
			a.HRef = String.Format("?q={0}{1}&amp;page={2}", Server.HtmlEncode(Request.QueryString["q"]), comment, (i + 1));
