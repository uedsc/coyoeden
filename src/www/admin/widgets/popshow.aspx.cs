...
public partial class admin_widgets_popshow : System.Web.UI.Page
{
	...
	private void RemoveWidget(string id, string zone)
	{
		...
		if (node != null)
		{
			...
			Setting.Delete(SettingTypes.Widget, id);
			Cache.Remove(String.Format("be_widget_{0}", id));
        ...
        if (data.Length == 4)
        {
            ...
            if (!widgetToMoveId.Equals(moveBeforeWidgetId))
            {
                ...
                if (oldZoneDoc != null && newZoneDoc != null)
                {
					...
					XmlNode widgetToMove = oldZoneDoc.SelectSingleNode(String.Format("//widget[@id=\"{0}\"]", widgetToMoveId));
