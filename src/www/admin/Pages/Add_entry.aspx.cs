...
public partial class admin_entry : System.Web.UI.Page, System.Web.UI.ICallbackEventHandler
{
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		if (!Page.IsPostBack && !Page.IsCallback)
		{
			...
			cbEnableComments.Enabled = BlogSettings.Instance.IsCommentEnabled;
	...
	private void valExist_ServerValidate(object source, ServerValidateEventArgs args)
	{
		...
		{
			if (cat.Name.Equals(txtCategory.Text.Trim(), StringComparison.OrdinalIgnoreCase))
				args.IsValid = false;
		}
	}
	...
	private void btnSave_Click(object sender, EventArgs e)
	{
		...
		post.IsCommentEnabled = cbEnableComments.Checked;
	...
	private void BindCategories()
	{
		foreach (Category cat in Category.Categories)
		{
			cblCategories.Items.Add(new ListItem(Server.HtmlEncode(cat.Name), cat.Id.ToString()));
		}
	}
	...
	private void BindPost(Guid postId)
	{
		...
		txtDate.Text = post.DateCreated.Value.ToString("yyyy-MM-dd HH\\:mm");
		cbEnableComments.Checked = post.IsCommentEnabled.Value;
		cbPublish.Checked = post.IsPublished.Value;
	...
	private void BindDrafts()
	{
		...
		foreach (Post post in Post.Posts)
		{
			if (!post.IsPublished.Value && post.Id != id)
			{
				...
				System.Web.UI.LiteralControl text = new System.Web.UI.LiteralControl(" by " + post.Author + " (" + post.DateCreated.Value.ToString("yyyy-dd-MM HH\\:mm") + ")");
