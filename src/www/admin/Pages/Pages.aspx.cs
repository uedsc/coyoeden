...
public partial class admin_Pages_pages : System.Web.UI.Page, System.Web.UI.ICallbackEventHandler
{
	...
	{
		Page page = CoyoEden.Core.Page.GetPage(pageId);
		if (page != null)
		{
	...
	{
		foreach (CoyoEden.Core.Page child in CoyoEden.Core.Page.Pages)
		{
			if (page.Id == child.ParentID)
			{
	...
	private void btnSave_Click(object sender, EventArgs e)
	{
		...
		if (Request.QueryString["id"] != null)
			page = CoyoEden.Core.Page.GetPage(new Guid(Request.QueryString["id"]));
		else
			page = new Page();
		...
		{
			foreach (Page otherPage in CoyoEden.Core.Page.Pages)
			{
				if (otherPage.IsFrontPage.Value)
				{
		...
		if (ddlParent.SelectedIndex != 0)
			page.ParentID = new Guid(ddlParent.SelectedValue);
		else
			page.ParentID = Guid.Empty;
	...
	private void BindPage(Guid pageId)
	{
		...
		cbIsFrontPage.Checked = page.IsFrontPage.Value;
		cbShowInList.Checked = page.ShowInList.Value;
		cbIsPublished.Checked = page.IsPublished.Value;
	}

	private void BindParents(Guid pageId)
	{
		foreach (Page page in CoyoEden.Core.Page.Pages)
		{
			if (pageId != page.Id)
		...
		{
			Page parent = CoyoEden.Core.Page.GetPage(pageId);
			if (parent != null)
	...
	private void BindPageList()
	{
		foreach (Page page in CoyoEden.Core.Page.Pages)
		{
			if (!page.HasParentPage)
			{
				...
				(" (" + page.DateCreated.Value.ToString("yyyy-dd-MM HH:mm") + ") ");
		...
		aPages.InnerHtml = CoyoEden.Core.Page.Pages.Count + " " + Resources.labels.pages;
	}

	private HtmlGenericControl BuildChildPageList(CoyoEden.Core.Page page)
	{
		HtmlGenericControl ul = new HtmlGenericControl("ul");
		foreach (Page cPage in CoyoEden.Core.Page.Pages.FindAll(delegate(CoyoEden.Core.Page p)
		{
			//p => (p.Parent == page.Id)))
		...
		}))
		{
			...
			(" (" + cPage.DateCreated.Value.ToString("yyyy-dd-MM HH:mm") + ") ");
