...
public partial class widgets_Twitter_widget : WidgetBase
{
    ...
    private TwitterSettings GetTwitterSettings()
    {
		...
		if (CurrentSettings.ContainsKey("accounturl") && !string.IsNullOrEmpty(CurrentSettings["accounturl"]))
        {
			Uri.TryCreate(CurrentSettings["accounturl"], UriKind.Absolute, out accountUrl);
            twitterSettings.AccountUrl = accountUrl;
        }

		if (CurrentSettings.ContainsKey("feedurl") && !string.IsNullOrEmpty(CurrentSettings["feedurl"]))
        {
			Uri.TryCreate(CurrentSettings["feedurl"], UriKind.Absolute, out feedUrl);
            twitterSettings.FeedUrl = feedUrl;
        }

		if (CurrentSettings.ContainsKey("pollinginterval") && !string.IsNullOrEmpty(CurrentSettings["pollinginterval"]))
        {
            int tempPollingInterval;
			if (int.TryParse(CurrentSettings["pollinginterval"], out tempPollingInterval))
            {
		...
		if (CurrentSettings.ContainsKey("maxitems") && !string.IsNullOrEmpty(CurrentSettings["maxitems"]))
        {
            int tempMaxItems;
			if (int.TryParse(CurrentSettings["maxitems"], out tempMaxItems))
            {
                if (tempMaxItems > 0)
		...
		if (CurrentSettings.ContainsKey("followmetext") && !string.IsNullOrEmpty(CurrentSettings["followmetext"]))
			twitterSettings.FollowMeText = CurrentSettings["followmetext"];
        else
