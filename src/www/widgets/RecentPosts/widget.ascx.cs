...
public partial class widgets_RecentPosts_widget : WidgetBase
{
	...
	public override void LoadWidget()
	{
		...
		if (CurrentSettings.ContainsKey("numberofposts"))
			numberOfPosts = int.Parse(CurrentSettings["numberofposts"]);

		if (HttpRuntime.Cache["widget_recentposts"] == null)
		{

			List<Post> visiblePosts = Post.Posts.FindAll(p => p.IsVisibleToPublic);
		...
		string content = RenderPosts((List<Post>)HttpRuntime.Cache["widget_recentposts"], CurrentSettings);
	...
	private string RenderPosts(List<Post> posts, StringDictionary CurrentSettings)
	{
		if (posts.Count == 0)
		{
		...
		foreach (Post post in posts)
		{
			...
			if (CurrentSettings.ContainsKey("showcomments") && CurrentSettings["showcomments"].Equals("false", StringComparison.OrdinalIgnoreCase))
				showComments = false;

			if (CurrentSettings.ContainsKey("showrating") && CurrentSettings["showrating"].Equals("false", StringComparison.OrdinalIgnoreCase))
				showRating = false;

			if (!showComments || !BlogSettings.Instance.IsCommentEnabled)
				comments = null;
