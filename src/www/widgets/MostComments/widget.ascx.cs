...
public partial class widgets_Most_comments_widget : WidgetBase
{
	...
	private void LoadSettings()
	{
		try
		{
			if (CurrentSettings.ContainsKey("avatarsize"))
				AVATAR_SIZE = int.Parse(CurrentSettings["avatarsize"]);

			if (CurrentSettings.ContainsKey("numberofvisitors"))
				NUMBER_OF_VISITORS = int.Parse(CurrentSettings["numberofvisitors"]);

			if (CurrentSettings.ContainsKey("days"))
				DAYS = int.Parse(CurrentSettings["days"]);

			if (CurrentSettings.ContainsKey("showcomments"))
				SHOW_COMMENTS = CurrentSettings["showcomments"].Equals("true", StringComparison.OrdinalIgnoreCase);
		}
		catch
		{ }
	}
	...
	private void BuildList()
	{
		...
		foreach (Post post in Post.Posts)
		{
			...
			{
				if (comment.DateCreated.Value.AddDays(DAYS) < DateTime.Now || string.IsNullOrEmpty(comment.Email) || !comment.Email.Contains("@"))
					continue;
