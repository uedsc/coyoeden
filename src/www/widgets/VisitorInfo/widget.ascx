...
<asp:PlaceHolder runat="Server" ID="phScript" Visible="false">
  ...
  <script type="text/javascript">
		function checkApml()
		{
			...
			CoyoEden.createCallback(CoyoEden.webRoot + 'widgets/visitor info/apmlchecker.ashx?url=' + apml_url, 
			function(response) 
			{
				if (response.length > 10)
				{
					...
					text += '<a href="javascript:void(location.href=\''+CoyoEden.webRoot+'?apml='+encodeURIComponent(response)+'\')">Apply filter</a>';
					CoyoEden.$('visitor_widget_apml').innerHTML = text;
				}
			});
		}
