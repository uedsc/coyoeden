...
public partial class page : CoyoEden.Core.Web.Controls.BlogBasePage
{
	...
	{
		this.Page = CoyoEden.Core.Page.GetPage(id);
		...
		CoyoEden.Core.Page.OnServing(this.Page, arg);
	...
	private void DeletePage(Guid id)
	{
		...
		{
			CoyoEden.Core.Page page = CoyoEden.Core.Page.GetPage(id);
			page.Delete();
			page.Save();
	...
	private void InjectUserControls(string content)
	{
		...
		foreach (Match myMatch in myMatches)
		{
			...
			{
				divText.Controls.Add(new LiteralControl(String.Format("ERROR - UNABLE TO LOAD CONTROL : {0}", myMatch.Groups[1].Value)));
			}
	...
	/// </summary>
	public new CoyoEden.Core.Page Page;
