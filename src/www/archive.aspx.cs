...
public partial class archive : CoyoEden.UI.SiteBasePage
{
	...
	private static void CreateTableRow(HtmlTable table, Post post)
	{
		...
		date.InnerHtml = post.DateCreated.Value.ToString("yyyy-MM-dd");
		...
		if (BlogSettings.Instance.EnableRating)
		{
			...
			rating.InnerHtml = post.Raters == 0 ? "None" : Math.Round(post.Rating.Value, 1).ToString();
