...
<asp:Content ID="Content1" ContentPlaceHolderID="cphBody" Runat="Server">
  ...
  <script type="text/javascript">
    ...
    {
    	if (CoyoEden.$('<%=txtAttachment.ClientID %>') && CoyoEden.$('<%=txtAttachment.ClientID %>').value.length > 0)
        return true;
        
      if(!Page_ClientValidate('contact'))
        return false;
        
      var name = CoyoEden.$('<%=txtName.ClientID %>').value;
      var email = CoyoEden.$('<%=txtEmail.ClientID %>').value;
      var subject = CoyoEden.$('<%=txtSubject.ClientID %>').value;
      var message = CoyoEden.$('<%=txtMessage.ClientID %>').value;
      var sep = '-||-';
      var arg = name + sep + email + sep + subject + sep + message;
      WebForm_DoCallback('__Page', arg, endSendMessage, 'contact', null, false) 
      
      CoyoEden.$('<%=btnSend.ClientID %>').disabled = true;
    ...
    {
      CoyoEden.$('<%=btnSend.ClientID %>').disabled = false;
      var form = CoyoEden.$('<%=divForm.ClientID %>')
      var thanks = CoyoEden.$('thanks');
