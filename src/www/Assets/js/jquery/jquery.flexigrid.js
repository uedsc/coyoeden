...
(function($) {
	...
	$.addFlex = function(t, p) {
		...
		p = $.extend({
			...
			onRowDblClick: false//row doubleclick event handler
		}, p);
		...
		var g = {
			...
			addData: function(data) { //parse data
				...
				p.CurData = data; //record the current raw data,should i?
			...
			populate: function() { //get latest data
				...
				if (p.onLoading) { p.onLoading(this); }; //levin090707
				...
				//Modified by Levin 07/25/2008
				if (p.urlType && p.urlType == "asmx") {
					//1,url is a asp.net webservice,we should pass a json like parameter
					param = { page: p.newp, rp: p.rp, sortname: p.sortname, sortorder: p.sortorder, query: p.query, qtype: p.qtype };
					if (p.criterias) { param.criterias = p.criterias; }; //additional criterias
					if (p.params) {
						param = $.extend(param, p.params);
					};
					var clientData = { Params: param };
					param = $.toJSON(clientData);
				} else {
					//2,url is common url,we pass querystring like parameters.
					param = [
				...
				$.ajax({
					...
					data: param,
					dataType: p.dataType,
					contentType: p.contentType || "application/x-www-form-urlencoded", //Added by Levin 07/24/2008
				...
				$('tbody tr', g.bDiv).each
					(
						function() {
							...
							.dblclick(//row double click event handler 080729
								function(e) {
									if (p.onRowDblClick) {
										var key = (this.id) ? this.id.substring(3) : ""; //remove id prefix 'row'
										p.onRowDblClick({ id: key, row: this });
									}
								}
							);
			...
			getSelectedRows: function() {
				var tempRows = $('tbody tr.trSelected', g.bDiv);
				var tempRowIDs = $.map(tempRows, function(n, i) {
					return n.id.substring(3);
				});
				var retVal = [];
				if (!p.CurData) return retVal;
				retVal = $.grep(p.CurData.rows, function(item, index) {
					return $.inArray(item.id, tempRowIDs) > -1;
				});
				return retVal;
			}, //endof getSelectedRows
			getModel: function(rowCell) {
				if (p.colModel) {
					var mObj = {};
					$.each(p.colModel, function(i, n) {
						mObj[this.name] = rowCell[i];
					});
					return mObj;
				};
				//no colModel
		...
		if (p.buttons) {
			...
			for (i = 0; i < p.buttons.length; i++) {
				...
				if (!btn.separator) {
					...
					btnDiv.innerHTML = "<div><span>" + btn.display || btn.name + "</span></div>";
					if (btn.bclass)
						$('span', btnDiv)
							.addClass(btn.bclass)
							.css({ paddingLeft: 20 })
					...
					if (btn.onpress) {
						$(btnDiv).click
							(
								function() {
									var rowsData = g.getSelectedRows();
									this.onpress({
										grid: g,
										name: this.name,
										rows: $('tbody tr.trSelected', g.bDiv), //selected rows,row's id is the datakey
										rowsData: rowsData
									});
								}
							);
						...
						.hover(
							function() {
								...
								if (g.colCopy) {
									...
									var onl = parseInt($('div:eq(' + nv + ')', g.cDrag).css('left')) || 0; //onl may be NaN in ie
