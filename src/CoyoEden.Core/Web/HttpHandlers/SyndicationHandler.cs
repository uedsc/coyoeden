	...
	public class SyndicationHandler : IHttpHandler
	{
		...
		private static List<IPublishable> GenerateItemList(HttpContext context)
		{
			...
			if (!string.IsNullOrEmpty(context.Request.QueryString["post"]))
			{
				...
				return post.Comments.ConvertAll(new Converter<IComment, IPublishable>(ConvertToIPublishable));
			}
				...
				try
				{
					using (WebClient client = new WebClient())
					{
						...
						using (System.IO.Stream stream = client.OpenRead(context.Request.QueryString["apml"]))
						{
							...
							list.Sort((i1,i2)=> { return i2.DateCreated.Value.CompareTo(i1.DateCreated.Value); });
							return list;
						}
					}
				}
		...
		{
			List<IComment> temp = new List<IComment>();

			foreach (Post post in Post.Posts)
			{
				foreach (var comment in post.ApprovedComments)
				{
					temp.Add(comment);
				}
			}
			...
			return list.ConvertAll(new Converter<IComment, IPublishable>(ConvertToIPublishable));
		}
		...
		private static void SetHeaderInformation(HttpContext context, List<IPublishable> items, SyndicationFormat format)
		{
			...
			{
				if (item.DateModified.Value.AddHours(-BlogSettings.Instance.Timezone) > lastModified)
					lastModified = item.DateModified.Value.AddHours(-BlogSettings.Instance.Timezone);
			}
