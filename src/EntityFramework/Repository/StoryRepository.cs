    ...
    public partial class StoryRepository : BaseRepository<IStory, Story>, IStoryRepository
    {
...
#endif
        {
            ...
            var stories = (DataContext != null)
                            ...
                            : Database.StoryDataSource
                                      .Where(s => (s.ApprovedAt != null) && s.Tag.Any(t => t.Id == tagId))
                                      .OrderByDescending(s => s.CreatedAt)
                                      .Skip(start)
                                      .Take(max);
...
#endif
        {
            ...
            var stories = (DataContext != null)
                            ...
                            : Database.StoryDataSource
                                      .Where(s => (s.ApprovedAt != null) && s.Tag.Any(t => t.Name == tagName))
                                      .OrderByDescending(s => s.CreatedAt)
                                      .Skip(start)
                                      .Take(max);
...
#endif
        {
            ...
            int total = Database.StoryDataSource
                .Count(s => (s.ApprovedAt != null) && (Database.StorySearchResult.Any(r => r == s.Id) || s.Category.Name.Contains(query) || s.Tag.Any(t => t.Name.Contains(query))));

            var stories = Database.StoryDataSource
                                      .Where(s => (s.ApprovedAt != null) && (Database.StorySearchResult.Any(r => r == s.Id) || s.Category.Name.Contains(query) || s.Tag.Any(t => t.Name.Contains(query))))
                                      .OrderByDescending(s => s.PublishedAt)
                                      .ThenBy(s => s.Rank)
                                      .ThenByDescending(s => s.CreatedAt)
                                      .Skip(start)
...
#endif
        {
            ...
            int total = Database.StoryDataSource
                                .Count(s => ((s.ApprovedAt != null) && s.StoryVote.Any(v => v.User.Id == userId)));
...
#endif
        {
            ...
            int total = Database.StoryDataSource
                                .Count(s => ((s.ApprovedAt != null) && s.StoryVote.Any(v => v.User.UserName == userName)));
...
#endif
        {
            ...
            var ids = Database.StoryDataSource
                              .Where(s => ((s.ApprovedAt != null) && s.StoryComment.Any(c => c.User.Id == userId)))
                              .Select(s => s.Id);
            int total = ids.Count();
...
#endif
        {
            ...
            return (DataContext != null)
                ...
                : Database.StoryDataSource.Count(s => (s.ApprovedAt != null) && s.Tag.Any(t => t.Id == tagId));
        }
...
#endif
        {
            ...
            return (DataContext != null)
                ...
                : Database.StoryDataSource.Count(s => (s.ApprovedAt != null) && s.Tag.Any(t => t.Name == tagName));
        }
