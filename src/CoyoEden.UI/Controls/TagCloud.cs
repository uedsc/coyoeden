...
namespace CoyoEden.UI.Controls
{
  public class TagCloud : Control
  {
    ...
    static TagCloud()
    {
      Post.Saved1+= delegate { _WeightedList = null; };
    }
    ...
    public override void RenderControl(HtmlTextWriter writer)
    {
			...
			{
				writer.Write("<p>" + Utils.Translate("none", "none...") + "</p>");
			}
      ...
      {
        writer.Write("<li>");
		writer.Write(string.Format(LINK, String.Format("{0}?tag=/{1}", Utils.RelativeWebRoot, HttpUtility.UrlEncode(key)), WeightedList[key], "Tag: " + key, key));
        writer.Write("</li>");
      }
