	...
	public class RelatedPosts : Control
	{
		...
		static void Post_Saved(object sender, SavedEventArgs e)
		{
			if (e.Action == SaveAction.Update)
			{
				...
				if (_Cache.ContainsKey(post.Id.Value))
					_Cache.Remove(post.Id.Value);
			}
		}
		...
		private string _Headline = Utils.Translate("relatedPosts", "related Posts");
		...
		public override void RenderControl(HtmlTextWriter writer)
		{
			...
			if (!_Cache.ContainsKey(Item.Id.Value))
			{
				lock (_SyncRoot)
				{
					if (!_Cache.ContainsKey(Item.Id.Value))
			...
			writer.Write(_Cache[Item.Id.Value].Replace("+++", this.Headline));
		}
		...
		private void CreateList(List<IPublishable> relatedPosts)
		{
			...
			sb.Append("</div>");
			sb.Append("</div>");
			_Cache.Add(Item.Id.Value, sb.ToString());
		}
