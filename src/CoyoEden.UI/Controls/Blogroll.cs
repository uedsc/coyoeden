...
namespace CoyoEden.UI.Controls
{
  ...
  /// </summary>
    public class Blogroll : Control
    {
            ...
            if (_Items == null)
            {
                lock (_SyncRoot)
                {
                    if (_Items == null)
                    {
                        ...
                        foreach (CoyoEden.Core.BlogRollItem roll in CoyoEden.Core.BlogRollItem.BlogRolls)
                        {
                            AddBlog(roll);
                        }
                    }
        ...
        private HtmlGenericControl BindControls()
        {
            ...
            foreach (blogRequest item in _Items)
            {
				...
				image.Alt = String.Format("RSS feed for {0}", item.RollItem.Title);
        ...
        /// </summary>
        private static void AddBlog(CoyoEden.Core.BlogRollItem br)
        {
            blogRequest affected = null;
            foreach (blogRequest req in _Items)
            {
    ...
    {
        internal BlogRollItem RollItem;
