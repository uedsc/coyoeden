...
namespace CoyoEden.UI.Controls
{
	...
	public class PostCalendar : Calendar, ICallbackEventHandler
	{
		...
		protected override void Render(HtmlTextWriter writer)
		{
			if (!ShowPostTitles)
			{
				...
				if (VisibleDate.Year != oldest.Year || VisibleDate.Month != oldest.Month)
					writer.Write(String.Format("<a href=\"javascript:CoyoEden.Calendar.nav('{0:yyyy-MM-dd}')\">{1}</a>&nbsp;&nbsp;", this.VisibleDate.AddMonths(-1), HttpUtility.HtmlEncode(PrevMonthText)));
				else
					writer.Write(HttpUtility.HtmlEncode(PrevMonthText) + "&nbsp;&nbsp;");

				writer.Write(String.Format("</td><td style=\"text-align:center;width:100px\">{0:MMMM yyyy}</td><td align=\"right\">", VisibleDate));

				if (VisibleDate.Year != DateTime.Now.Year || VisibleDate.Month != DateTime.Now.Month)
					writer.Write(String.Format("&nbsp;&nbsp;<a href=\"javascript:CoyoEden.Calendar.nav('{0:yyyy-MM-dd}')\">{1}</a>", this.VisibleDate.AddMonths(1), HttpUtility.HtmlEncode(NextMonthText)));
				else
					writer.Write(String.Format("&nbsp;&nbsp;{0}", HttpUtility.HtmlEncode(NextMonthText)));
		...
		private DateTime GetOldestPostDate()
		{
			...
			{
				return Post.Posts[Post.Posts.Count - 1].DateCreated.Value;
			}
...
function setupCoyoEdenCalendar() {
  CoyoEden.Calendar = {
  	  months: {},
	  nav: function(date) {
		  var m = CoyoEden.Calendar.months;
		  if (m[date] == null || m[date] == 'undefined')  {
		  	  " + Page.ClientScript.GetCallbackEventReference(this, "date", "CoyoEden.updateCalendar", "date") + @"
		  } else {
			  CoyoEden.updateCalendar(m[date], date);
		  }
	  }
  };
}
			...
			sb.Append("{" + Page.ClientScript.GetCallbackEventReference(this, "date", "CoyoEden.updateCalendar", "date") + "}");
			sb.Append("else {CoyoEden.updateCalendar(months[date], date)}");
			sb.Append("}");
			sb.Append("</script>");
			*/
			return sb.ToString();
