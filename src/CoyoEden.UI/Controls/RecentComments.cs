...
namespace CoyoEden.UI.Controls
{
	...
	public class RecentComments : Control
	{
		...
		private static List<IComment> _Comments = new List<IComment>();
		...
		private string RenderComments()
		{
			...
			{
				return "<p>" + Utils.Translate("none", "none...") + "</p>";
			}
			...
			foreach (var comment in _Comments)
			{
				if (comment.IsApproved.Value)
				{
					...
					if (comment.Website != null)
					{
						...
						LiteralControl wrote = new LiteralControl(String.Format(" {0}: ", Utils.Translate("wrote", "wrote")));
						li.Controls.Add(wrote);
					}
					else
					{
						LiteralControl author = new LiteralControl(String.Format("{0} {1}: ", comment.Author, Utils.Translate("wrote", "wrote")));
						li.Controls.Add(author);
					}
          ...
          if (commentBody.Length > 0)
          {
            if (commentBody[commentBody.Length - 1] == '&')
            {
              commentBody = commentBody.Substring(0, commentBody.Length - 1);
					...
					link.InnerHtml = String.Format("[{0}]", Utils.Translate("more", "more"));
