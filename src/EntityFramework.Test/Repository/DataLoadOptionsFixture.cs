...
namespace Kigg.Infrastructure.EF.Test
{
    ...
    public class DataLoadOptionsFixture
    {
        ...
        {
            _loadOptions.LoadWith<Category>(c => c.Story);
            Assert.Throws<InvalidOperationException>(() => _loadOptions.LoadWith<Category>(c => c.Story));
        }

        [Fact]
        public void IsPreloaded_Should_Return_True_When_Related_EntityCollection_Is_Registered_Using_LoadWith()
        {
            _loadOptions.LoadWith<Story>(s => s.Tag);
        ...
        {
            _loadOptions.LoadWith<User>(u => u.Tag);
